...
void Flock::Render(float deltaT)
{
	...
	for (auto& pAgent : m_Agents)
	{
		if (pAgent != m_Agents[0]) pAgent->SetBodyColor(m_StandardBodyColor);
	}
...
void Flock::RenderNeighborhood()
{
	...
	if (m_IsSpacePartitioningActive)
	{
		m_pCellSpace->RenderNeighborsAgent(m_Agents[0], m_NeighborhoodRadius);
	}
	else
	{
		RegisterNeighbors(m_Agents[0]);
		// DebugRender the neighbors in the memory pool
		DEBUGRENDERER2D->DrawCircle(m_Agents[0]->GetPosition(), m_Agents[0]->GetRadius(), Elite::Color{ 1.f, 0.f, 0.f }, DEBUGRENDERER2D->NextDepthSlice());
		DEBUGRENDERER2D->DrawCircle(m_Agents[0]->GetPosition(), m_NeighborhoodRadius, Elite::Color{ 1.f, 1.f, 1.f }, DEBUGRENDERER2D->NextDepthSlice());

		for (int index{}; index < m_NrOfNeighbors; ++index)
		{
			DEBUGRENDERER2D->DrawCircle(m_Neighbors[index]->GetPosition(), m_Agents[0]->GetRadius(), Elite::Color{ 0.f, 1.f, 0.f }, DEBUGRENDERER2D->NextDepthSlice());
		}
	}
...
void Flock::InitializeAgents()
{
	...
	m_StandardBodyColor = m_Agents[1]->GetBodyColor();
